...
uv run python examples/feibi_zh_tts.py
```

也可以直接使用脚本参数指定，脚本会在导入 `genie_tts` 之前设置环境变量：

```bash
uv run python examples/feibi_zh_tts.py --genie-data-dir D:\path\to\GenieData
```
...
```bash
uv run python examples/feibi_zh_tts.py --cli
```

自定义文本和输出路径：

```bash
uv run python examples/feibi_zh_tts.py --text "你好，我是菲比。欢迎使用 Genie 生成中文语音。" --output outputs/demo.wav
```

生成后直接播放：

```bash
uv run python examples/feibi_zh_tts.py --play
```

如果当前安装的 `genie-tts` 版本公开了 `download_roberta_data()`，可以额外下载 RoBERTa 文本特征资源：

```bash
uv run python examples/feibi_zh_tts.py --download-roberta
```
